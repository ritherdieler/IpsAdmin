...
class TechnicianViewModel : ViewModel() {
    ...
    fun registerTechnician(
        ...
        if (lastName.isEmpty()) {
            technicianFromErrorLiveData.postValue(TechnicianFromError.OnEtLastNameError("El apellido del tecnico no " +
                    "puede estar " +
                    "vacio"))
            return
        }
        if (dni.isEmpty()) {
            technicianFromErrorLiveData.postValue(TechnicianFromError.OnEtDniError("El numero de dni no " +
                    "puede estar " +
                    "vacio"))
            return
        }
        if (type.isEmpty()) {
            technicianFromErrorLiveData.postValue(TechnicianFromError.OnEtTypeError("el typo de tecnico no " +
                    "puede estar " +
                    "vacio"))
            return
        }
        if (userName.isEmpty()) {
            technicianFromErrorLiveData.postValue(TechnicianFromError.OnEtUserNameError("El Usuario no " +
                    "puede estar " +
                    "vacio"))
            return
        }
        if (password.isEmpty()) {
            technicianFromErrorLiveData.postValue(TechnicianFromError.OnEtPasswordError("La contrasena no  " +
                    "puede estar " +
                    "vacio"))
            return
        }
        ...
        if (address.isEmpty()) {
            technicianFromErrorLiveData.postValue(TechnicianFromError.OnEtAddressError("La direccion no" +
                    "puede estar " +
                    "vacio"))
            return
        }
        if (phone.isEmpty()) {
            technicianFromErrorLiveData.postValue(TechnicianFromError.OnEtPhoneError("El numero de telefono no " +
                    "puede estar " +
                    "vacio"))
            return
        }
        ...
        viewModelScope.launch {
            try {
                ...
                technicianResponseLiveData.postValue(TechnicianResponse.OnTechnicianRegistered(technicianFromRepository))
